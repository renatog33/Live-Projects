...
In addition , I used C# to display the "Time Unitl Start/Time Left" for the rental.  Also, a button was created to use onclick Javascript to toggle between "Current rentals" and "Expired rentals".
...
<div class="RentalRequest-index--toggleContainer">
    <input class="RentalRequest-index--toggleBtn" type="button" id="expiredBtn" value="Expired Rentals" onclick= "HideAndShow()" />
</div>
<br>
<br>
<div class="RentalRequest-index--accordionContainer">
    <div class="accordion" id="accordionExample">
        @foreach (var item in Model.OrderBy(item => item.StartTime.Date).ThenBy(item => item.StartTime.TimeOfDay))
    {  <!-- //sort the rental requests by date -->
      ...
      {
        <div class="RentalRequest-index--current card" id="current">
            <div class="RentalRequest-index--card-header" id="headingOne">
                <h5 class="mb-0">
                    <button class="RentalRequest-index--cardBtn notExpired" type="button" data-toggle="collapse" data-target="#info-@item.RentalRequestId" aria-expanded="true" aria-controls="collapseOne">
                  ...
                  {
                            <p class="RentalRequest-index--timeUntil">Time Left: @remainingDiff.Minutes Minutes</p>
                  }
      ...
      {
        <div class="RentalRequest-index--expired card" id="expired">
            <div class="RentalRequest-index--card-header" id="headingOne">
                <h5 class="mb-0">
                    <button class="RentalRequest-index--cardBtn" type="button" data-toggle="collapse" data-target="#info-@item.RentalRequestId" aria-expanded="true" aria-controls="collapseOne">
                        <div class="RentalRequest-index--headBox">
                            ...
                            <p class="RentalRequest-index--timeUntil">Expired</p>
                        </div>
                    </button>
                </h5>
            </div>
...
<script>
//ensures that no expired rental requests are displayed when the page first loads.  
  $(document).ready(function () {
  $(".RentalRequest-index--expired").hide();
  });

//toggle between expired and current rentals
  var switcherDisplay = true;
  function HideAndShow() {
    console.log(switcherDisplay);
    var x = $(".RentalRequest-index--expired");
    var k = $(".notExpired");
    try {
      switch (switcherDisplay) {
        case true:
          x.css('display', 'block');
          k.css('display', 'none');
              switcherDisplay = false;
              $("#expiredBtn").val("Current Rentals");
          break;
        case false:
          k.css('display', 'block');
          x.css('display', 'none');
              switcherDisplay = true;
              $("#expiredBtn").val("Expired Rentals");
          break;
      }
    } catch(e) {
